#!/usr/bin/env node
/**
 * narrate-pipeline.mjs · L2 long voiceover commander
 *
 * Input: markdown voiceover draft (## scene-id segment, [[cue:id]] marks key sentences)
 * Output: voiceover.mp3 (the entire spliced ​​vocal segment) + timeline.json (the absolute time of each segment start/end + cues)
 *
 * Usage:
 *   node scripts/narrate-pipeline.mjs --script demo.md --out-dir _narration_demo
 *
 * voiceover manuscript format:
 *   ---
 * title: What is LLM
 ...
 *
 *   ## intro
 * Hello everyone, my name is Huashu. Today we will explain what LLM is in 5 minutes.
 *
 *   ## what-is
 * LLM stands for Large Language Model, [[cue:bigmodel]] It is a neural network with hundreds of billions of parameters.
 *Essence is a predictor of text solitaire.
 *
...
narrate-pipeline.mjs · L2 long voiceover commander
  ...
  --script <path> voiceover draft .md file (required)
...
function splitByCues(text) {
  ...
  // Filter empty text blocks (when the cue is close to the beginning/end of the paragraph)
...
async function main() {
  ...
  if (scenes.length === 0) {
    console.error('Error: The voiceover draft does not have a ## scene section, at least one section.');
    process.exit(1);
  }
  ...
  for (let i = 0; i < scenes.length; i++) {
    ...
    // Splicing to the main track: first add gap (except the first segment), then add scene
    if (i > 0 && gap > 0) {
  ...
  console.error(`number of cues: ${totalCues}`);
}
