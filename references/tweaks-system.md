...
The mark block has no effect in the localStorage scheme (just an ordinary comment), but will be read in the host that supports source code writeback to achieve source code level persistence. Plus this is harmless to the current environment while remaining forward compatible.
