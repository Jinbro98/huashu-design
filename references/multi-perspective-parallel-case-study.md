...
> Parallel director's notes + HTML + keyframe experiments from 6 artist perspectives

---

## background

When the user requested "make a 30-second upgrade promotional video for huashu-md-html v2.0", the main thread first produced the v5 baseline (Anthropic / Penguin Classics publishing house grade). But the user thought it could be done better and gave a critical instruction:
...
Don’t pick 6 designers at random – they must be highly visually differentiated to avoid convergence.

The 6 perspectives finally selected (including reasons for selection):

| Perspective | Genre | Aesthetic anchor | Differences from other perspectives |
|------|------|---------|----------------|
| **v5 Baseline** | Modern Press | Anthropic terra cotta + Penguin Classics serif + Vignelli grid | A safe "tasteful" choice |
...
| **v5c Wong Kar-Wai** | Hong Kong-style new wave | "In the Mood for Love" and "2046" letterboxing + Chinese serif | Slow shot + fog halo + Chinese-based |
...
4. **Each has a visual signature that is “completely opposite to the common SaaS aesthetic in the training corpus”**
...
3. What you need to do (4 deliverables)
4. The artist’s DNA (6 core fields):
   - Swatches (specific HEX)
   - Font (specific name + alternative)
   - Visual language (core items)
   -Identifiable signatures
   - Rhythm (difference from other perspectives)
   - Enhanced version of anti-AI slop (a no-no in the context of this style)
5. 30 seconds of structure reference (4-6 shot drafts)
6. Design requirements for destination cards (keep them real and readable)
7. Key constraints (30s / 1920×1080 / file:// / Google Fonts CDN)
8. Output verification checklist + complete report format
```

**Key**: Each brief must emphasize "**Don't repeat the aesthetics of v5**" - otherwise the subagent will be influenced by the v5 director-notes and converge.
...
- Fixed v5 baseline bug in main thread
-Write review framework (dimensions/Q&A for each version)
- Precipitation methodology to skill (this is the source of this case study)
- Prepare final summary document skeleton

### Step 4 · Failure handling (about 16% failure rate, acceptable)

Practical observation: About 1 of the 6 subagents will fail due to network or token exceeding the limit (Bass first round socket error). deal with:

1. Immediately check the output folder of the agent when receiving the completion notification
...
```
5 Dimension Rating (1-10 per dimension):
...
- Q1 Share screenshots? (Can trigger pause on social platforms)
- Q2 Remember a sentence? (Can leave proposition-level memory)
- Q3 Cross-era? (Looking back in 5 years, it doesn’t look cheap)

use case distribution (by platform and audience):
- WeChat Official Account / X / Bilibili / Moments / Dribbble / Customer Demo / Private Domain / ...
```
...
- 5 Mostly structurally complete: version 6/6
...
- Average size of each PNG: 100-200KB

### Duration

- 6 subagents running in parallel: about 12-15 minutes (duration_ms shown)
- Main thread parallel idle work (v5 repair + writing methodology): completed at the same time
- Overall "from starting 6 perspectives to all deliverables in place": about 60 minutes
...
Each of the six subagents produced a complete spec of 4,000-12,000 words according to the 5-most structure, and all achieved marketing-ready quality when implementing HTML. This proves that the methodology itself does not rely on the talent of a single executor - **as long as the brief is given clearly, multiple independent executors can produce consistent high-quality results**.

### Insight 2 · "Perspective" must be specific to "work + year"

Each brief lists specific dialogues about the work:
...
After adding these, the 6 versions have extremely high stylistic purity, and none of them converge with each other.

### Insight 4 · The real value of multiple perspectives is not “selecting winners”

The original idea was to A/B test to select the best version. During the actual review, I found that: **6 versions each have a clear use case**:
- v5 baseline → Product page / WeChat reading (high information density)
- Anderson → WeChat Official Account long post header picture (strong sense of flipping through magazines)
- WKW → Bilibili / Chinese culture (nostalgic temperature)
...
1 out of 6 failed (Bass first round socket error). Processing cost: restart + 5 minutes of simplified version brief, then wait 12-15 minutes. **Comparison vs. waiting for 1 agent to run 6 versions sequentially (90+ minutes)** - Parallel + retry is obviously more economical.

### Insight 6 · The main thread must do substantive idle work while waiting

The subagent takes 12-15 minutes to complete. The main thread should never be idle during this time:

- **Main version bug** (already reported by users)
- **Write review framework** (fill in when waiting for review)
- **Precipitation methodology into skill** (such as this case study)
...
| Project ready for multi-platform distribution (X / WeChat Official Account / Bilibili / Moments) | ✅ Enable | One version per platform |
...
[6 subagents parallel]
   ...
   └── v5f brief → ...
       ↓
[Main thread synchronization] Fix v5 bugs · Write review framework · Precipitation methodology
       ↓
[Full 6 notifications arrive]
       ↓
[Main thread] Failure detection + retry / screenshot replacement
       ↓
[Main thread] 5-dimensional scoring + 3 top-level questions + use case allocation
       ↓
[Main thread] Write final REVIEW.md
       ↓
[Delivery] 6 full versions + review + platform distribution recommendations
```
...
- Actual project location: author's local demos directory (including a full set of 6 + 1 perspective files, not distributed with the warehouse)
- Review review: Author's local REVIEW.md (not distributed with the warehouse)
