...
> Color decision making §1.a Core asset protocol (extracted from brand spec) or “design direction advisor”
...
//Corresponding to CSS: cubic-bezier(0.16, 1, 0.3, 1)
Easing.expoOut(t) // = t === 1 ? 1 : 1 - Math.pow(2, -10 * t)

// 2. Overshoot · Flexible toggle/button popup
//Corresponding to CSS: cubic-bezier(0.34, 1.56, 0.64, 1)
Easing.overshoot(t)
...
**Specific color value decision** go to §1.a core asset agreement (extracted from brand spec) or "design direction advisor"
...
// After the focus is completed, do a 150ms Flash highlight at the focus position to guide the line of sight back.
...
// ↓ Same as layoutId after clicking
...
<Sprite start={8} end={8.5}>
  {/* 0.5s pause - nothing moves, letting the audience stare at the loading state */}
  <LoadingState />
</Sprite>
...
```js
// Complete three segments of card entry
const anticip = interpolate(t, [0, 0.2], [1, 0.95], Easing.easeIn); // preparation
const action = interpolate(t, [0.2, 0.7], [0.95, 1.05], Easing.expoOut); // Active
const settle = interpolate(t, [0.7, 1], [1.05, 1], Easing.spring); // Rebound
// Final scale = three-segment product or segmented application
```
...
4. When exporting video, go to `audio-design-rules.md` + `sfx-library.md`

---

## Appendix · Source of material for this document

- Anthropic official animation dismantling: `Reference Animation/BEST-PRACTICES.md` in the Huashu project directory
