...
Most SaaS upgrade videos make the same mistake—treating footage as PPT. Open → slide through 6 functions → logo + slogan → end. Every second is "showing" and not a second is "telling". What the audience remembers when they leave is not the product, but “another page that looks like it was made by AI.”
...
That film is the ceiling of technology company promotional videos in my mind. Director Mark Romanek did three things right:
...
Our film has no voiceover, so the first two principles need to be strengthened to 200% by typography and timing.
...
We inherit this set of colors. But do it **more restrained** - Anthropic occasionally uses pure terracotta orange for large color blocks; our terracotta orange is always only used as accent (accounting for < 8% of the total image area), leaving 92% for ivory white and jet black.
...
The slogan reveal at 25-29 seconds uses this language: **ONE SOURCE.** and **SIX FORMS.** are not "decorative text", they are the picture itself.
...
They open the movie to find out "what's been upgraded." Our promise to them: Within 30 seconds, you have to know for sure—
...
→ Shot 08 and Shot 09 are 3 seconds each and must have a "★ NEW" label + **visible professional-level details** such as "printer's crop mark" and "Apple Books frame" must be visible on the destination card - allowing old users to quickly understand that "this is not a round-up function, it is a serious job."

**Second Audience B · AI Native creators who have heard of huashu-md-html but have no use for it (about 25%)**
...
**External audience C · Completely unfamiliar designers/editors/publishers (about 15%)**
...
- The stuff this company makes has **publishing taste**
- Different from the AI ​​tools you have seen in the past

→ The anti-AI slop self-test (Part II.7) of the entire chip is done for them. Any purple gradients, emoji icons, SVG hand-drawn characters - none of them appear.

## 1.4 Rhythm Philosophy

The rhythm of Apple's promotional videos is not uniform. It is a **slow beat-acceleration-peak-slow-down** curve (see Part III Emotional Curve Chart for details).
...
- **6-22s second acceleration segment**: 6 capabilities are completed in one go, and each one lasts 3 seconds without letting go.
...
## 1.5 What this slop doesn’t do (anti-AI slop self-test)

In order of importance:

| Not to do | Reason |
|------|------|
| No need for purple gradient | The universal formula of "technical sense" in the training corpus, it will be cyber slop in 2026 |
| Don’t use emojis for icons | The disease of “using emojis if you are not professional” |
...
| No stock photos | No real photos appear in the entire video (it's about typography, not lifestyle) |
| Do not draw progress bar + time code + copyright signature bar | These are player chrome, not content chrome - it will conflict with external players |
...
Not 3 colors, but 10 colors. Each color has a **function definition** (not "just use it if it looks good").
...
Charred #2A2620 Very dark brown black (for cover card only) < 5%
...
**Iron Rule**:
- No color other than the above 10 colors will appear in any shot. **There is no "temporarily add some cold gray to this shot"**.
- The total of the three colors of terracotta orange (Terracotta + variants) accounts for < 10% of the screen, otherwise it will be visually overloaded.
- Any text can only use one of 4 colors: Ink / Cinder / Smoke / Terracotta.
...
- Centerline: y = 540 (hero md default anchor)
- Golden section upper: y = 412
- Under the golden section: y = 668

**Key Security Zone**:
- Within the top 60px: chrome element area (capability counter, version chip)
...
**Stagger’s Law**:
- When multiple elements enter the scene at the same time, adjacent elements will delay 30-80ms (not 0, nor more than 100ms)
- 6 pills enter the field: cumulative stagger 250ms (50ms each)
...
- The last shot within the last 300ms: opacity 1 → 0, scale 1 → 0.96
- Next shot within the first 300ms: opacity 0 → 1, scale 1.04 → 1
- The two shots overlap for 300ms (on the timeline, the Sprite end is 0.3s larger than the start of the next shot)
...
- Use Terracotta + bold for the current capability, and Smoke for other capabilities.
- A horizontal line connects 6 names, and the progress point (●) slides from left to right over time
- Entrance: 07s, full fade-in 500ms
...
- Completely static, not moving

**Chrome E · very light paper texture (continuous)**

- SVG noise + extremely slow 0.3% scale breathing
- opacity ≤ 0.04
- Almost invisible when recording, but it allows the picture to "breathe"
...
**Levels (each level lasts 30 seconds, intensity changes are controlled by envelope)**:
...
□ Terracotta orange takes up < 10% of the frame in this mirror
□ This shot has at least one "pause worthy screenshot" detail (120% signature)
□ The transition from the previous shot to this shot is cross-dissolve + scale, not hard cut
□ At the end of this shot, a visual "make way" for the next shot (not "filling the whole frame to the end")
```
...
The audience enters the picture. The question is raised: What is the source of truth?
...
The theme sublimates. slogan appears. Brand seal.
...
- **08.0s**: 6 files cards are collapsed into md → "Ah, it turns out that md is the source" click for the first time
- **18.0s**: The first NEW tag appears → old users "Oh"
- **22.5s**: All chrome is closed, ready to enter Act III → tension build-up peak
...
The entire 1920×1080 is Ivory paper #FAFAF6. **Nothing in the picture**.
...
- 0.50-1.50s · The entire screen is held, with no action. Allow the viewer's eyes to adjust to the white.
- 1.40-1.50s · A cursor position begins to appear in the center left of the screen (x=860, y=540) (transparent and will not be visible until the next shot)
...
This 1.5 seconds is one of the most important 1.5 seconds in the film.
...
After flashing twice (0.7s per cycle × 2), `# markdown.md` begins to appear word by word behind the cursor, with the font JetBrains Mono 56px, the color Ink #1A1A1A, and letter-spacing -0.01em.

Each time a character is typed, the keyboard click sounds once. After typing the last character (13 characters in total), the cursor continues to flash 1 time after `.md`.
...
- ✅ The font is JetBrains Mono, not Courier or Menlo, which is the system default mono.
- ✅ pause-and-look signature: cursor's 3px width (not 2px or 4px) - a very precise detail, knowledgeable people will notice that this is "real terminal design"

**[WHY]**

This mirror is the core of setup: **markdown is not a noun, it is an action** - it is the matter of "typing the keyboard to turn characters into structures" itself.

A cursor is the smallest unit of writing. Starting from a cursor is the birth of "source code".
...
**[TIMECODE]** 03.00 — 05.00s (2.0s) `|` **FUNCTION** Reveals hero. `# markdown.md` morphs into hero `md.`
...
- 04.80-05.00s (200ms): period officially settles, hero complete. A 320px wide terracotta orange thin line (terracotta accent rule, 2px thick) appears 30px below, spreading out from the center to both ends.
...
- 30px below accent rule, width 320px (grows from 0)
...
- ✅ morph is not a cheap transition of "fade out + fade in", it is a real character deformation (including ghost afterimage superposition)
- ✅ The period is the hero's "signature detail" (the one that 120% does it): Terracotta's period is as small as a fingernail, but it is the visual anchor of this film, **This period remains as the hero's logo in all subsequent shots**
...
- ✅ pause-and-look signature: 480px Newsreader 'md' has a kerning of -0.04em, which makes the m and d almost fit but not touch. This is the signature texture of the Newsreader font in large sizes.
...
When the next shot enters ACT II, ​​the hero is already standing - it will be pushed to the top of the screen to make room for "materialized products".
...
05.00s: hero `md.` slides up from the center of the screen (y=540) to y=280 (i.e. 1/4 height position), while shrinking to 220px.
...
```
.pdf │ Two-column layout + header "doc.pdf" + page number "— 12 —" + a few lines of small text for real typesetting
.docx │ heading "On Markdown" + subscript italic + 6-line paragraph ascii
.pptx │ Title "MD AS SOURCE" + a simplified bar chart placeholder
.xlsx │ 6×4 spreadsheet grid + some numbers
.epub │ Apple Books Feng’s Page + Chapter Title "Chapter 01"
...
- hero `md.`: reduced to 220px (same as SHOT 03 font size)
...
- 05.30s · Chrome A capability counter entry (CAPABILITY · 01 shown, first dot filled)
- 05.60-07.60s · 6 cards are launched in sequence (each launch delay = 5.60 + i × 0.18s, flight 1.1s, absorb at launch+1.1)
...
- ✅ No glow or particle effects are given to the md character (no need to explain "md is absorbing", the audience can understand it themselves)
- ✅ pause-and-look signature: When you pause and look at each card during the flight, you can read "This is a PDF / This is a DOCX" - this is the detail that is 120% achieved.
- ✅ tagline uses "→" instead of "to" or "to", which is markdown's own character

**[WHY]**

This is the door mirror for the ACT II. If the audience doesn't realize "Oh, md is the source" after watching these 3.5 seconds, the subsequent shots will be in vain.
...
3. Return all to md - "But they are all md in the end"
...
08.50s: hero `md.` slides from the top of the center to the left side of the screen (x=480, y=540), keeping the size at 220px.
...
- ✅ "article.html · TUFTE THEME" is a "detailed signature that can be read during pause"
...
3. Staggered entry inside the destination card (each card has 6-8 text levels)
4. The card content is real and readable, not fake bar lines

The audience will understand this pattern after seeing it for the second time (SHOT 06), and will have the feeling of "Ah, here we go again, but this time it's NEW" after seeing it for the sixth time (SHOT 09) - this is exactly the rhythm design of ACT II.
...
cross-dissolve entry. The destination card in the previous shot shrinks and exits to the lower right corner within 11.50-11.80s, and the new destination card (this time showing markdown source code) enters from the right.
...
**Key differences** (forms visual rhythm with SHOT 05):
...
- card is a deep base (visual contrast, emphasizing that this is source)
...
- 13.10-13.80s · markdown internal 6 lines staggered entry (100ms delay per line)
  - Special micro-detail: Simulate typewriter when each line enters - line's character-by-character cascade reveal (making the audience feel that "this is the process of markdown being "written"")
- 13.80-14.50s · hold
...
- ✅ The reverse arrow is not a "U-turn curve", it is a straight line + reverse arrow - maintain structural consistency
...
This is why I chose 02 (md→HTML) → 03 (HTML→md) → 04 (md→docx) for the capability sequence - deliberately placing the reverse capability in the 3rd position to maximize the cognitive surprise of the "two-way flow".
...
Back to the shallow bottom, back to "md on the left, destination on the right".

destination card design: **publisher-grade docx chapter homepage** (high density of information, but complete restraint).
...
- Terracotta full-width rule (not a thin line, but a 3px thick rule) is the signature of the publishing house’s chapter home page
...
- 14.50-14.80s · Previous shot card exit + md stay
- 14.80-15.10s · arrow line forward growth
- 15.10s · arrow head, label "md → docx" enters
- 15.30-16.10s · Destination card for overall admission
...
- ✅ Do not write the explanation of "This is a mockup of the inside page of a book" (let the layout speak for itself)
...
After watching this shot, the audience is ready for the chain "md → print". The NEW tags of the next two shots will be taken over.
...
cross-dissolve entry. The visual intensity of this shot is **significantly higher** than SHOT 05-07 - because this is "new stuff" and needs to be remembered.
...
2. **The destination is not a single card, but two PDF fans come out**: A4 is at the back (slight +5° rotation), and large 32K (176×240mm, domestic paper book specifications) is at the front (slightly -3° rotation), forming a vision of "both page-sizes are supported"
3. **Each PDF has "printing crop marks" (crop marks)** - a small L-shaped line in each corner, 2px thick, Smoke color - this is the detail of the real printer's PDF
4. All arrow + label colors are Terracotta (not Ink), and the overall color is warmer
...
- 17.50-17.80s · Previous shot card exit + md stay
...
- 18.90-19.70s · Two PDFs of internal cascade staggered admission
- 19.70s · 4 crop marks (PDF B) appear in sequence (80ms cascade, sign the details of the "printing factory process")
- 19.70-20.50s · hold
...
- C: Progress point ● Slide to the "md→pdf" position, the text size at this position will be increased by 0.5px for emphasis
- D: ON
- E: ON
...
- ✅ pause-and-look signature: The "Large 32K · 176×240mm" at the top of PDF B is mixed with Chinese and English, which is Huashu Ecosystem's respect for domestic paper book specifications.
...
NEW tags + crop marks + two PDF fans + complete A4 / Large 32K specification instructions - four things together to achieve the above two things.
...
- The entire card gives a sense of reality that "I am reading this book in Apple Books"
...
- 20.50-20.80s · Previous scene PDF exit + md remain
- 20.70s · NEW label **remains lit** (it will not pop up again this time because it has been created in SHOT 08 - just display "★ NEW" directly)
...
- B: The orange dot pulse of version chip is enhanced (amplitude × 1.5)
...
- ✅ Do not draw the real logo of Kindle or Apple Books (to avoid IP risks); just use macOS window chrome to imply "reader"
- ✅ No e-ink gray filter (avoid Kindle slop)
- ✅ "Apple Books · 1 of 24" chrome is a real publishing data sense (24 chapters, Chapter 1)
...
In the center of the screen, the md character slides from the left position (x=480) back to the center (x=960), while the size changes from 220px → 300px.

The six capability labels around md (any→md / md→HTML / HTML→md / md→docx / md→pdf / md→epub) emerge one by one from a distance (circle r=380px), forming a circle around the md character, one every 60°, arranged in clockwise order (starting from "any→md" at the top). These labels are a mix of Mono Bold 14px Smoke (not active) + Terracotta (actually new).

Overall effect: The **md characters are the sun and the 6 capabilities are the planets. **

But this shot doesn’t need to hold the viewer in for too long – it’s a transitional shot.
...
- A: fade out at 22.50s (counter is 6/6, mission accomplished)
- B: ON, but starts preparing the transition for ACT III (keeps the same position, but tightens the internal spacing slightly)
...
- ✅ The 6 capability labels are not "circling around the md" (to avoid the cyber slop of the "planet spinner"); they are "settling at a fixed position and then fading together" (more restrained)
...
ACT II ends at 22.50s (NEW (06) has just been completed), but the slogan still needs to enter the scene at 24.00s - the 1.5s in the middle cannot be "blank waiting", there must be narrative motion.
...
In the next shot, give way to slogan. The md character is reduced to 180px and is ready to become the "brand seal" of the slogan.
...
The md character has been reduced to 180px and stays in the center of the screen (y=540).
...
- **SFX: impact (slogan ONE) at 24.20s — deep bass impact + short reverb tail (-8dB)** ← This is the strongest SFX cue on this film
- SFX: extremely light pen-on-paper stroke at 26.00s (when rule appears, -22dB)

**[CHROME]**

- A: OFF (has exited)
- B: ON, but **important change**: version chip now cross-dissolve into a new form - in the same position in the upper right corner, but the size of the chip is slightly larger, the font size is 18px (previously 16px), more prominent. At the same time, the pulse amplitude of Terracotta dot × 2 (emphasis on "v2.0 upgrade time")
- C: OFF (has exited)
- D: ON
- E: ON

**New chrome**:
- The md character (top-left, 56px, Newsreader 600 + Terracotta dot) officially settled in corner and became the brand seal

**[ANTI-SLOP]**

- ✅ The slogan is not "whole word fade-in" (cheap), but letter-by-letter stagger (movie quality)
...
- ✅ The font size of 168px is verified by layout - no matter how big it is, it will collide with SIX FORMS. (SHOT 12), if it is smaller, it will not be powerful enough.
...
It is strategic to have the md character retreat to the top-left at this moment - the slogan is the protagonist and md is the brand seal. The two don't compete with each other.
...
The lower part of the screen (y=720) begins to enter the hero slogan bottom line:
...
27.20-27.80s: 30px below SIX FORMS., 6 capability pills appear, entering in order:
...
27.80-28.30s: The sub-bidding bank enters the market (y=890):
...
- BGM: peak swell continues at 26.50s and reaches the loudest (-4dB) in the whole film at 27.20s
...
- ✅ Use "," Chinese comma + period "." as subscript - it is respect for Chinese typesetting
- ✅ This frame (28.30s) is the "most complete frame for marketing" of this film - you can screenshot it as thumbnail / X poster / WeChat Official Account cover image, all information is in one frame: slogan + 6 capability + sub-logo + brand seal + version

**[WHY]**

This is the resolution mirror.

If SHOT 11 is thesis (ONE SOURCE.), SHOT 12 is antithesis + synthesis (SIX FORMS. plus full capability map).

At frame 27.50s, the audience should be fully absorbed by typography while listening to the string peak - this is the most worthwhile 5 seconds of this film.
...
There are only 4 elements in the entire picture: md seal, accent rule, top-right chrome, and bottom-right watermark. All others are empty.
...
- 29.00-29.20s · Hold the previous shot (let the audience absorb it completely)
...
- ✅ No need for sign-off text such as "Thank you" or "Made with love" (cheap)
...
- ✅ pause-and-look signature: The md of the last frame. Under the 88px Newsreader font, the Terracotta dot is the visual focus of the entire screen - the audience's eyes will naturally stay on this dot, and then see the accent rule below, and then to the top-right version chip. This "line of sight" is the success of visual hierarchy design
- ✅ silence leaves breathing room for the screen in the last 0.2s
...
If there is one frame in the entire film that the audience will remember, I hope it is this frame.
...
**Preferred**: Use Suno v6.0 / Udio v1.5 to generate a 30-second cinematic minimal piece yourself, prompt keywords:
...
**MINIMUM STANDARDS**: BGM 30 seconds in length, 44.1kHz sampling rate, aim for -16 LUFS integrated loudness.

## 5.4 SFX Source

**Preferred**: Use huashu-design skill’s 37 pre-made assets in `assets/sfx/<category>/*.mp3`:

```
Event Recommended SFX Files
─────────────────────────────────────────────────────
...
The following keyframes must be validated after implementing HTML (with Playwright + `?t=NN` URL parameter):
...
- Word spacing and line height visually correct
...
v5-final.GIF social media version (30s, palette optimized, < 8MB, used for WeChat Official Account embedding)
v5-final-silent.MP4 Silent version (backup to facilitate subsequent dubbing/BGM change)
v5-poster.png poster version (taken from this frame at t=28.5s, used for X card/WeChat Official Account cover)
...
If a specific decision cannot be traced back to this sentence, don't make it.

---

*Director's notes — end of document*
*Total word count: about 11500 Chinese words*
*Next: After passing the user review, enter the HTML implementation stage*
