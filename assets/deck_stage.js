 ...
 * - Click left and right for area navigation
 * - slide counter (current/total)
 * - localStorage persists the current slide
 * - Speaker notes postMessage (supports outer rendering)
 * - Hash navigation (#slide-5 jump to the 5th picture)
 * - Print-to-PDF support (Cmd+P / Ctrl+P one slide per page)
 * - Automatically add data-screen-label to each slide
 *
 * Usage:
 *   <deck-stage>
...
(function() {
  ...
  class DeckStage extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Shadow DOM is rendered first (independent of child nodes and not affected by parser timing)
      this._render();

      // Defense: If the script is placed in <head> (not after </deck-stage>),
      // The parser may not have finished processing the subsection <section> at this moment, and querySelectorAll will return empty.
      // Delay to the next event loop to ensure that all child nodes have been parsed.
      const init = () => {
        this._collectSlides();
      ...
      if (this.ownerDocument.readyState === 'loading') {
        // The document is still in parse, waiting for DOMContentLoaded to finish all sections at once
        this.ownerDocument.addEventListener('DOMContentLoaded', init, { once: true });
      } else {
        // The document has been parsed (script is at the bottom of the body or defer), just collect it in the next frame
